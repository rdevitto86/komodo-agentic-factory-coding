...
- **`swe`** owns dev-time security: input validation, authz patterns, secret handling, secure coding per `~/.claude/agents/swe/security.md`. That stays with `swe`. When a finding requires a code fix, hand it to `swe`.
...
Hard rules: `~/.claude/agents/swe/principles.md` — error strings, scope discipline, no commits/branches. If this agent writes code (PoC exploits, tooling, scripts), comment rules from `~/.claude/agents/swe/comments.md` apply. The dev-time security baseline this agent builds on: `~/.claude/agents/swe/security.md`.
