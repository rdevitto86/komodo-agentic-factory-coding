...
Builds on `~/.claude/agents/swe/go/coding.md` and `~/.claude/agents/swe/ts/coding.md` — same bar, Python idioms. Idiomatic Python you already know is assumed; this covers only Komodo-specific or non-obvious points. Project configs may extend, not contradict.
...
- Docstrings never open with the function name (`comments.md`). Boolean prefixes `is_/has_/can_/should_`; `req`/`res` for HTTP request/response.
