...
`pytest` only (no `unittest`). Only unit tests colocate as `test_<name>.py` in the same package. Integration tests move to a top-level `test/integration/` or `tests/integration/` (pick one spelling per repo, stay consistent), flat by feature — not mirroring the package tree, e.g. `test/integration/test_order.py`. Fixtures over class inheritance; `@pytest.mark.parametrize` over in-test loops. Mock at module boundaries (`monkeypatch`/`unittest.mock`), never internals. Async via `pytest-asyncio` (`asyncio_mode = "auto"`). DB-touching code → integration tests against ephemeral instances (`testcontainers`, `pytest-postgresql`), placed under the integration tier folder. Coverage >80% business logic, 100% security-critical.
