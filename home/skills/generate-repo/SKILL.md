...
**Refresh diffs the repo's actual layout and conventions against the language skill's `Repo layout — <token>` tree.** Only what differs becomes a Deviations line. Matching it produces zero lines, not a line saying so.
