...
argument-hint: [slice id or task, or "open <topic>" for the unscripted path]
...
`$ARGUMENTS` names the slice or task. **If it begins with `open`, skip the whole machine** — see The open hatch at the bottom.
...
**Run `/decompose [target state]`.** It reads the repo facts, the SDD's slice table, the backlog, and the changelog in a fork, and returns a queue.
...
**Run `/implement <task text, slice id, and Done when command>`**, once per task.
...
**Run `/consolidate <the slice ids and task summaries that went green>`** once the whole band is done, not after each task.
...
| Need | Send to |
|---|---|
| Read-only research across many files | `engineering` / `business` |
