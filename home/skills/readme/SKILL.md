...
- Load `tech-stack` for repo-kind detection and the standard layout.
- Detect the language from the repo root and load the matching language skill for its Quick-reference fields (port, entrypoint, build/run/test commands).
- If `docs/prd.md` / `docs/sdd.md` exist, load `docs` and pull from them — never invent what they don't state.
- If `TODO.md` exists, check for open Blocker-tier items.
...
- **1. Overview** — one paragraph on what it is. If there's a common misreading of its role (e.g. it looks like it does X but actually only does Y), one line stating what it deliberately is *not*. If `TODO.md` has an open Blocker, one status line here too, pointing at `TODO.md` — omit entirely on a clean repo.
...
- **5. References** — pointer table to what exists: `docs/prd.md`, `docs/sdd.md`, `docs/adr/`, `TODO.md`, `openapi.yaml` or equivalent contract file. List only files present in this repo.
