...
- **Zero comments. Ever. No exceptions — but never remove one you didn't author either.** No function/method/class docs, no declaration comments (type, struct, interface, field, var, const), no file/package headers, no inline or trailing notes — in any language. The "why / public API / edge case" licenses are retired. Toolchain directives, test banners, and user-requested comments are always fine. This is a two-way constraint, not just a ban on adding: a pre-existing comment you didn't write is never yours to delete, even when you're editing that exact line for an unrelated reason. Only the user removes their own comments. If you accidentally strip one, put it back with a normal edit — the hook only blocks genuinely new comments, not a line that already existed before your change. Full rule: `~/.claude/standards/comments.md`.
