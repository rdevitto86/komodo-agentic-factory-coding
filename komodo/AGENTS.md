...
## Git
- **Inside your worktree you are free.** Reset, checkout, restore, rebase, or delete your branches; nothing is precious.
...
- **Landing is the human's merge button.** Hand the user any refused command; after a merge, run `komodo sync` yourself.
- Past a one-line fix, run the line: `/run <group>` in session, `komodo run <group>` headless.
