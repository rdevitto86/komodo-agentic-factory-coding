...
## Worker output
Return only the JSON object the schema describes: a one-line `summary` and a `findings` array.

## Session output
Return a table `Sev | File:line | Class | Claim | Fix`, then one line of summary. Nothing else.
