...
The boxes gate `backlog-audit`'s sweep, not `workflow-implement`'s completion check — a `[DONE]` task carrying any unchecked `- [ ]` AC box is verdicted Ambiguous and left unswept, on the theory that a task can pass every subtask's `Done when` command and still not have proven the behavior its own Acceptance Criteria describe. `workflow-implement` is the only thing that ticks a box (once the subtask proving that AC goes green), so a task that states Acceptance Criteria with no subtask ever mapped to one is a backlog defect, not a sweep the gate should quietly let through.
