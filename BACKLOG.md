...
- [H] The `chore/workflow-loop-round-cap-retry-counter` branch's committed diff (against `main`) contains only `docs/design-decisions.md` and a partial `BACKLOG.md` edit — the actual TSK-01.4.2/TSK-01.4.5 round-cap prose in `ways/sdlc.md` and the TSK-01.4.3 retry-counter wiring in `SKILL.md`'s P2.1/P2.3/P2.4 exist only as uncommitted working-tree edits, so the branch as it stands ships none of the reviewed feature and the work is one `git checkout`/reset away from silent loss; TSK-01.4.2/TSK-01.4.3/TSK-01.4.5 also remain `[TODO]` in `BACKLOG.md` even though their `Done when` greps already pass against the uncommitted files · S → commit `ways/sdlc.md` + `SKILL.md`, then mark TSK-01.4.2/TSK-01.4.3 (SUB-01.4.3.2)/TSK-01.4.5 `[DONE]`
...
#### [TSK-01.4.12] `git_guard.py`'s `MAX_SCAN_DEPTH` bound is enforced by three separately hand-rolled `if depth > MAX_SCAN_DEPTH: raise/append` checks (`resolve_command_output`, `expand_word`, `scan_command`) instead of one shared guard — the next recursive scanner added to this file has nothing to copy but the same three-line pattern again [P: M] [TODO]
* **SUB-01.4.12.1** consolidate the three depth checks into one helper (e.g. a small `check_depth(depth)` raising `ScanDepthExceeded`, called at the top of each recursive function) so a future recursive scanner has one call to add, not a pattern to reinvent
  * **Done when:** `/assess-simplify claude-code/hooks/git_guard.py` reports it clear

#### [TSK-01.4.13] `is_guarded_path`'s `repo_root_of` shells out to `git rev-parse --show-toplevel` on every call with no memoization, and it's called once per source argument in a multi-source `cp`/`mv` (and once per redirect/tee target) — an `mv a b c d e /repo/dir` spawns a `git` subprocess per source for a value that's identical across the whole scan [P: M] [TODO]
* **SUB-01.4.13.1** cache `repo_root_of` (e.g. `functools.lru_cache`) or hoist the repo-root resolution to once per top-level `scan_command` call instead of once per candidate path, so a multi-argument write command doesn't spawn a redundant `git` process per argument
  * **Done when:** `/assess-simplify claude-code/hooks/git_guard.py` reports it clear

#### [TSK-01.4.14] `scripts/test-hooks.sh`'s new smoke-test cases S4 (`git push origin main`) and S6 (`sed -i -e s/a/b/ file`) run the byte-identical command string already asserted by G58 and G107 respectively — the smoke set adds no coverage those two exercise beyond running earlier in the file [P: L] [TODO]
* **SUB-01.4.14.1** swap S4/S6 to a distinct representative command each (or accept the overlap explicitly with a comment noting the smoke set intentionally duplicates a couple of G-cases for fast-fail ordering) so the two blocks aren't silently asserting the same thing twice
