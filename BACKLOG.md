...
#### [TSK-01.1.20] `scripts/test-hooks.sh`'s plain `bash_case`/`smoke_case` invocations (`S1`/`S5`-`S7`, `G100`, `G125`-`G131`, `G134`-`G135`, etc.) sent no `cwd` in the payload, so `git_guard.py`'s cwd-derived `is_guarded_path`/`repo_root_of` resolved against `os.getcwd()` of whatever process invokes the test script — these deny assertions only passed because the suite happens to be run from inside this repo's own git worktree; run from a checkout without `.git` (a tarball, a stripped CI workspace) or any other non-repo cwd, and every one of them would silently flip from deny to allow-then-fail, unlike the `bash_case_at` cases that already pinned cwd explicitly [P: M] [DONE]
* **SUB-01.1.20.1** pin `cwd` explicitly in `bash_case`/`smoke_case` (a fixed fixture repo, top-level `cwd` in the payload — `git_guard.py` reads `payload["cwd"]`, not `tool_input.cwd`), matching the existing `bash_case_at` pattern of never depending on the suite's own invocation cwd
  * **Done when:** `/assess-bugs scripts/test-hooks.sh` reports it clear

- [L] `scripts/test-hooks.sh` now pins cwd two different ways for the same purpose: `bash_case`/`smoke_case` set a top-level `"cwd"` field in the JSON payload, while the pre-existing `bash_case_at` relies on `git_guard.py`'s `payload.get("cwd") or os.getcwd()` fallback by `cd`-ing the subshell into the fixture dir before invoking the hook — both work, but the file now carries two mechanisms for "pin the effective cwd" instead of one · S → `/assess-simplify scripts/test-hooks.sh` reports it clear
