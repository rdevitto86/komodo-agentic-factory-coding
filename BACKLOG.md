...
#### [TSK-01.3.2] Bring workflow-loop under the 5,000-token compaction re-attach cap and make the gates survive compaction [P: M] [DONE]
...
* **SUB-01.3.2.4** [High] found reviewing the trim: SUB-01.3.2.1's own instruction was to cut justification prose while keeping every rule, but the Guardrails section's bridge bullet lost an actual directive, not just rationale — before: "**The bridge is optional, never blocking.** An unreachable MCP server is a skipped step. Never branch a phase on whether it is up." After: "**The bridge is optional, never blocking** — an unreachable MCP server is a skipped step." The sentence "Never branch a phase on whether it is up." is a standalone rule (what not to do), not an explanation of why, and it is gone from the file entirely — a future compaction-shortened read of this skill no longer carries that prohibition.
  * **Done when:** `grep -q 'Never branch a phase on whether it is up' claude-code/skills/workflow-loop/SKILL.md`
