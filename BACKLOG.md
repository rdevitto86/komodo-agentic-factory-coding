...
#### [TSK-01.1.15] `git_guard.py`'s guarded-path redirect/tee check is bypassable via a command substitution in the redirect target — `REDIRECT.findall(mask_data(command))` matches only up to the first whitespace, so `$(echo BACKLOG.md)` or a backtick equivalent tokenizes as `$(echo`, which `is_guarded_path` never recognizes as guarded, while the shell still expands and redirects into the real guarded file; the same whitespace-split token miss lets `tee $(echo BACKLOG.md)` through `scan_segment`'s `tee` branch too — confirmed live: `echo bad >BACKLOG.md` is denied by the hook, `echo bad >$(echo BACKLOG.md)` and `tee $(echo settings.json)` return zero findings from `git_guard.scan_command` [P: C]  [TODO]
* **SUB-01.1.15.1** resolve a redirect/tee target through `extract_substitutions`'s captured substitution text (or reject any redirect/tee target containing an unresolved `$(`/backtick outright) before checking `is_guarded_path`, so a target computed via command substitution is checked against its expanded form, not its first whitespace-delimited fragment
  * **Done when:** `/assess-security claude-code/hooks/git_guard.py` reports it clear

#### [TSK-01.1.16] `claude-code/hooks/comments.py` defines `KNOWN_TEMPLATE_TYPES` twice in a row, identically — dead duplicate module-level constant [P: L] [TODO]
* **SUB-01.1.16.1** delete the second (redundant) `KNOWN_TEMPLATE_TYPES = (...)` assignment at `claude-code/hooks/comments.py:36`, keeping the first
  * **Done when:** `[ "$(grep -c '^KNOWN_TEMPLATE_TYPES = ' claude-code/hooks/comments.py)" = 1 ]`

#### [TSK-01.1.17] `claude-code/settings.json`'s `skillOverrides` carries dead entries for skills with no live `SKILL.md` on disk — `workflow-authoring` has no directory under `claude-code/skills/` at all, and `standards-c` is disabled (`SKILL.md.off`, no active `SKILL.md`) [P: L] [TODO]
* **SUB-01.1.17.1** remove the `workflow-authoring` key from `skillOverrides` (no corresponding skill exists), and either remove the `standards-c` key too or restore `claude-code/skills/standards-c/SKILL.md` if it should be active — pick per whichever is the intended state, not a blind delete
  * **Done when:** `bash scripts/validate.sh` reports no broken/unused `skillOverrides` entries

#### [TSK-01.1.18] `git_guard.py`'s guarded-path redirect check misses a quoted redirect target — `mask_data()` blanks every quoted span *before* `REDIRECT` runs, so `echo bad > "BACKLOG.md"` leaves only whitespace after the `>` and the capture group never matches, while `is_guarded_path`'s own `token.strip("\"'")` shows quoted targets were meant to be caught; confirmed live against the installed hook: `echo bad > BACKLOG.md` → DENY, `echo bad > "BACKLOG.md"` and `echo bad > 'BACKLOG.md'` → ALLOW (`tee "settings.json"` still denies, so the hole is specific to the redirect path, not the tokenized ones) [P: H] [TODO]
* **SUB-01.1.18.1** run `REDIRECT.findall` against a mask that preserves quoted redirect targets (or re-scan the raw command for a quoted target) so `> "BACKLOG.md"`, `> 'BACKLOG.md'`, and the `>>` forms all resolve to the same guarded path the bare form does
  * **Done when:** `bash scripts/test-hooks.sh` passes with a new case asserting `echo bad > "BACKLOG.md"` is denied

#### [TSK-01.1.19] `git_guard.py`'s in-place-write and trailer scanners run against the raw command instead of `mask_data(command)`, so merely *mentioning* one of their patterns inside a quoted string or heredoc body blocks the turn — `INPLACE_SED`, `INPLACE_PERL`, `PYTHON_WRITE` and `GIT_MESSAGE_CALL`/`BANNED_TRAILER` all `.search(command)` on the unmasked text while the `REDIRECT` check two lines below correctly uses `mask_data(command)`; confirmed live: `echo 'sed -i is mentioned here'` → DENY. Hit for real during this assessment — a heredoc containing `sed -i` as *test data* was refused, costing a full round trip [P: M] [TODO]
* **SUB-01.1.19.1** compute `mask_data(command)` once in `scan_command` and run `INPLACE_SED`, `INPLACE_PERL`, `PYTHON_WRITE` and the `GIT_MESSAGE_CALL`/`BANNED_TRAILER` pair against the masked text, keeping the raw command only where a check genuinely needs unmasked data
  * **Done when:** `bash scripts/test-hooks.sh` passes with new cases asserting `echo 'sed -i is mentioned here'` is allowed and a real `sed -i ... file` is still denied

#### [TSK-01.1.20] `comments.py check` in its default (changed-lines) mode never sees a new, untracked file — `changed_line_map` shells out to `git diff --unified=0 HEAD --`, which excludes untracked paths entirely, so a brand-new source file reports clean until it is staged or committed; `make comments`/`make verify` (the Stop gate's own target) therefore pass on exactly the case that most needs the lint. Confirmed live: an untracked `zz_gaptest.go` with a 3-value return reported `0 finding(s)` on the default check and `MISSING RET_ARITY_3` under `--all` [P: M] [TODO]
* **SUB-01.1.20.1** fold untracked-but-not-ignored files into `changed_line_map`'s result as wholly-changed (`git ls-files --others --exclude-standard`), so a new file is linted in full without switching the whole check to `--all`
...
#### [TSK-01.1.21] This repo has no CI — `.github/` holds only `PULL_REQUEST_TEMPLATE.md`, with no workflow running `make verify`, so the 160-case hook suite, `scripts/validate.sh`, and the comment lint only ever run on the author's own machine via `scripts/hooks/git/pre-push-verify`; a push that bypasses the local hook (or a contributor who never ran `scripts/hooks/git/install.sh`) lands unverified config that every project's next session picks up [P: M] [TODO]
* **SUB-01.1.21.1** add `.github/workflows/verify.yml` running `make verify` on pull requests and pushes to `main`, matching the contract `standards-cicd` already states for a target repo
  * **Done when:** `test -f .github/workflows/verify.yml && grep -q "make verify" .github/workflows/verify.yml`

#### [TSK-01.1.22] The entire hook layer depends on `python3` being on `PATH` and nothing declares or checks it — all four `settings.json` hook commands and both `scripts/hooks/git/` dispatchers shell out to `python3`, `setup.sh` never mentions it, and there is no manifest declaring a floor; if `python3` is missing or shadowed, the hook command itself fails before any Python runs, so `git_guard.py`'s fail-closed `except BaseException` handler never executes and the guard silently degrades open [P: M] [TODO]
* **SUB-01.1.22.1** have `setup.sh` verify `python3` resolves and meets the toolkit's actual floor before linking anything, and state that floor in the repo's `AGENTS.md` alongside the hook table
  * **Done when:** `bash setup.sh --dry-run` reports the detected `python3` version and fails when none resolves

#### [TSK-01.1.23] `git_guard.py`'s new `resolve_guard_target`/`shell_words` path (added to close TSK-01.1.15/18/19) only strips a matching quote pair from the *ends* of a word, unlike the `shlex`-based tokenizer it replaced for `tee`/redirect scanning — a mid-word quote split (e.g. `Docker"file"`, which the shell dequotes to `Dockerfile`) no longer resolves to the real filename, so a write to an extensionless guarded name in `FILENAME_FAMILY` (`Dockerfile`, `Makefile`, `Justfile`) bypasses the guard on both paths; extension-based guarded names (`.md`, `.json`) are unaffected since `os.path.splitext` still finds the trailing extension regardless of the embedded quote. Confirmed live against the installed hook: `tee Docker"file"` and `echo x > Make"file"` both return zero findings (exit 0, no denial), while `tee Dockerfile` and `echo x > Makefile` (no split) are correctly denied [P: H] [TODO]
* **SUB-01.1.23.1** in `resolve_guard_target` (or `is_guarded_path`), dequote a raw word the same way `shlex.split` would — strip and join *every* quoted run in the word, not just a single matching pair at its two ends — before the `FILENAME_FAMILY`/extension check, so `Docker"file"` resolves to `Dockerfile` on both the `tee` and redirect paths
  * **Done when:** `bash scripts/test-hooks.sh` passes with new cases asserting `tee Docker"file"` and `echo x > Make"file"` are denied

#### [TSK-01.1.24] `git_guard.py`'s `resolve_guard_target` only resolves a redirect/tee word that is *exactly* one recorded substitution span — a word built by concatenating a substitution with adjacent literal text, or containing a nested substitution, falls back to checking the still-unexpanded raw text instead, and the guarded-path match evaporates on both the redirect and `tee` paths [P: C] [TODO]
* **SUB-01.1.24.1** a word like `pre$(echo _BACKLOG.md)` bypasses the guard entirely: its span never matches the substitution's own recorded span (which covers only the `$(...)` portion), so `resolve_guard_target` falls through to `is_guarded_path(word)` against the literal, still-unresolved text `pre$(echo _BACKLOG.md)`, whose extension is `)` not `.md` — confirmed live: `echo bad > pre$(echo _BACKLOG.md)`, `echo bad > "pre$(echo BACKLOG.md)"`, and `tee pre$(echo _BACKLOG.md)` all return zero findings from `git_guard.scan_command`, while the shell itself would write to `pre_BACKLOG.md`, a real `.md` file the guard exists to protect
  * **Done when:** `/assess-security claude-code/hooks/git_guard.py` reports it clear
* **SUB-01.1.24.2** a nested substitution (`$(echo $(echo BACKLOG.md))`) matches its own outer span correctly, but `resolve_guard_target` recurses via `tokenize(sub_text)` — a plain `shlex.split` with no notion of `$()`/backtick boundaries — so the inner substitution's closing paren glues onto the token (`BACKLOG.md)`), and `is_guarded_path`'s extension check (`.md)` ≠ `.md`) misses it; confirmed live: `echo bad > $(echo $(echo BACKLOG.md))` and `tee $(echo $(echo BACKLOG.md))` both return zero findings
  * **Done when:** `bash scripts/test-hooks.sh` passes with new cases asserting both shapes above are denied

#### [TSK-01.1.25] `git_guard.py`'s `cp`/`mv` branch in `scan_segment` was never migrated to the `shell_words`/`resolve_guard_target` path that `tee` and the redirect scanner got — it still calls `parse_cp_mv_target`/positional-arg logic against `tokens`, which is built from `tokenize(segment)` on the *masked* segment where `extract_substitutions` has already blanked any `$(...)`/backtick span to whitespace, so a `cp`/`mv` destination built from a command substitution vanishes from `tokens` entirely and the real destination is never checked; confirmed live: `cp /tmp/a.txt $(echo handler.go)` and `mv /tmp/a.txt $(echo handler.go)` both return zero findings from `git_guard.scan_command`, while the shell itself would write `/tmp/a.txt`'s contents into `handler.go`, a guarded `.go` path — plain-quoted and unquoted destinations (`cp /tmp/a.txt "handler.go"`, `cp /tmp/a.txt handler.go`) are still caught correctly, so the hole is specific to the substitution case [P: C] [TODO]
* **SUB-01.1.25.1** rework the `cp`/`mv` branch to resolve its destination (and `-t`/`--target-directory` value, and each source) through `resolve_guard_target`/`shell_words` against the segment's raw span, the same way `tee` was rewired, instead of `tokenize`'s blanked-segment tokens
  * **Done when:** `bash scripts/test-hooks.sh` passes with new cases asserting `cp /tmp/a.txt $(echo handler.go)` and `mv /tmp/a.txt $(echo handler.go)` are denied

#### [TSK-01.1.26] `git_guard.py` has no handling anywhere for process substitution (`<(...)`/`>(...)`) — `extract_substitutions` only recognizes `` ` `` and `$(`, so a `>(...)` write target is never extracted as a substitution to recurse into, never blanked from the segment, and never reached by the redirect-operator or `tee` scans, which both look for a literal `>`/word boundary, not a `>(` process-substitution open; confirmed live: `echo x | tee >(cat > BACKLOG.md)` returns zero findings from `git_guard.scan_command`, while the shell itself opens a real writable fd to `BACKLOG.md` through the inner `cat >`. This is a pre-existing gap (process substitution was unhandled before this recursive-substitution rewrite too), not a regression it introduced, but it remains a live bypass of the same guarded-path check the rewrite hardened [P: H] [TODO]
* **SUB-01.1.26.1** teach `extract_substitutions`/`split_segments` (or a dedicated pass) to recognize `<(`/`>(` the same way `$(` is recognized, and recurse `scan_command` into the captured body so a nested redirect inside it is caught
  * **Done when:** `bash scripts/test-hooks.sh` passes with a new case asserting `tee >(cat > BACKLOG.md)` (or an equivalent process-substitution write) is denied

#### [TSK-01.1.27] `git_guard.py`'s redirect-operator regex (`REDIRECT_OPERATOR = r"(?<![-=<0-9&])>>?"`, carried unchanged through the TSK-01.1.15/18/19/23/24/25 hardening) still excludes any `>`/`>>` immediately preceded by a digit or `&`, so a real file-descriptor redirect — `1>file`, `2>file`, `9>file`, `&>file` — never reaches `resolve_guard_target` at all; confirmed live: `echo bad > BACKLOG.md` is denied by the hook, but `echo bad 1>BACKLOG.md`, `echo bad 2>BACKLOG.md`, `echo bad 9>BACKLOG.md`, and `echo bad &>BACKLOG.md` all return zero findings, while the shell itself opens the guarded file and writes to it in every case. Pre-existing (regex unchanged by this diff), not a regression it introduced, but it remains a live bypass of the exact guarded-path check the diff just hardened [P: H] [TODO]
* **SUB-01.1.27.1** narrow the lookbehind so it only excludes an actual fd-duplication target (`N>&M`), not every digit/`&`-prefixed redirect, so `1>`, `2>`, `9>`, and `&>` reach `resolve_guard_target` the same way a bare `>` does
...
#### [TSK-01.1.28] `git_guard.py`'s `INPLACE_SED`/`INPLACE_PERL` patterns require a literal `-i`/`--in-place` immediately after whitespace in the raw segment text, so quoting the flag defeats detection even though the shell still passes it through unchanged — confirmed live: `sed -i -e s/a/b/ file` is denied, but `sed "-i" -e s/a/b/ file` and `sed '-i' -e s/a/b/ file` both return zero findings, and the shell executes the identical in-place edit either way. Pre-existing (same regex, same raw-text matching, before and after this diff's move of the check into `scan_segment`), not a regression, but it directly undermines the guard TSK-01.1.19 just relocated [P: M] [TODO]
* **SUB-01.1.28.1** run `INPLACE_SED`/`INPLACE_PERL` against a dequoted view of the segment's arguments (e.g. via `shell_words`/`expand_word` on each token) instead of the raw quoted text, so a quoted `-i` is recognized the same way a bare one is
...
#### [TSK-01.1.33] `git_guard.py`'s `resolve_command_output` only recognizes a `$()`/backtick body it can pattern-match literally — when the leading word isn't `echo` and no argument's *raw text* contains a recognizable guarded filename/extension, it falls back to joining the unresolved argument text, so a captured command that *computes* the guarded name at runtime (rather than spelling it in its own argv) evades `is_guarded_path` even though the shell executes it and redirects into the real file; confirmed live: `echo bad > $(python3 -c "print('BACKLOG'+'.'+'md')")` returns zero findings from `git_guard.scan_command` (exit 0, no denial), while `bash -c` of the identical command actually overwrites `BACKLOG.md` — `$(printf "BAC""KLOG.md")` and other cases where the literal argv text still contains the guarded name/extension are correctly denied, so the hole is specific to a substitution whose output is assembled by the inner command itself, not by shell-level word-splitting/concatenation [P: H] [TODO]
* **SUB-01.1.33.1** since arbitrary command execution can't be statically resolved, treat an unresolved non-`echo` command substitution feeding a redirect/`tee`/`cp`/`mv` target as guarded by default (deny-by-default) rather than falling back to a literal-text scan of its argv, unless the substitution's own argv can be proven inert (e.g. `echo`/`printf`-only bodies already handled)
  * **Done when:** `/assess-security claude-code/hooks/git_guard.py` reports it clear
