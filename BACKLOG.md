...
- [M] Git hooks: `scripts/hooks/git/` still has no commit-time check for this repo's own `.py` hooks — `pre-push-verify` now covers push (runs `.claude/verify.sh` when present), but a staged syntax error in `git_guard.py`/`comment_guard.py` still passes `git commit` uncaught · M → a staged `.py` syntax error fails `git commit`
