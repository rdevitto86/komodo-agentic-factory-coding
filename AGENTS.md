...
Typed-only workflow skills: `/audit-readiness` · `/audit-change-risk` · `/audit-code-quality` · `/audit-testing` · `/audit-readme`. These stay `disable-model-invocation: true` — a human decision to normalize a backlog or run an audit should start from the user, not the model's own judgement.

`audit-performance`, `backlog`, `git-commit-message`, `audit-bugs`, `audit-security`, `audit-simplify`, `repo-init`, `write-readme`, and the four mid-loop phases (`workflow-decompose`, `workflow-implement`, `workflow-consolidate`, `workflow-complete`) all carry neither key: `/audit-code-quality` invokes the first internally as part of its conventions pass, `workflow-loop`'s P1 invokes the second (`backlog audit`) before the decompose fork, `workflow-loop`'s P3 invokes the third directly (not inside the `workflow-consolidate` fork, which is read-only git) to commit the band once consolidate returns, `ways/sdlc.md` invokes the next three from P2.3/P2.4, and `workflow-loop`'s P0, `/workflow-consolidate`'s README-refresh step, and `/repo-init`'s Scaffold/Refresh path all invoke `write-readme` — a `disable-model-invocation: true` skill cannot be reached by another skill's instructions, only by the user typing its name, and several of these run inside forked, unattended phases so none of them can carry that key. **`disable-model-invocation` is the only thing that controls cross-skill reachability — it says nothing about listing cost.** None of these thirteen is ever chosen by the model reading its description out of a bare, undirected request: each is either named explicitly by an already-loaded parent skill's instructions or typed directly, so all thirteen collapse to `name-only` in `skillOverrides`. `workflow-loop` is the sole exception, kept at full description because it must fire from plain language alone with nothing already loaded to name it. The trade is the same one `repo-init` makes below: each pays full description cost so it stays callable by name.
...
|---|---|---|---|
| Command — produces an artifact | `write-<noun>` | User invokes it to create/refresh a deliverable | `write-readme` |
| Command — scores, finds, or files | `audit-<noun>` | User invokes it, or the loop invokes it mid-band, to score a diff or walk a diff/repo for defects — findings are filed to `BACKLOG.md` as stories unless `--report` is passed | `audit-bugs`, `audit-change-risk`, `audit-code-quality`, `audit-performance`, `audit-readiness`, `audit-readme`, `audit-security`, `audit-simplify`, `audit-testing` |
...
- **Typed-only workflow skills cost nothing** — `disable-model-invocation: true` keeps `/audit-readiness`, `/audit-change-risk`, `/audit-code-quality`, `/audit-testing`, and `/audit-readme` out of the listing entirely.
- **Everything reached only by an explicit name is `name-only`, full stop.** The four mid-loop phases (`/workflow-decompose`, `/workflow-implement`, `/workflow-consolidate`, `/workflow-complete`), `/repo-init`, `/write-readme` (also `paths`-triggered on `README.md`, so it still expands in full the instant the model is about to touch that file), the six audit/commit command skills (`/audit-performance`, `/backlog`, `/git-commit-message`, `/audit-bugs`, `/audit-security`, `/audit-simplify`), and `/git-pr-create` carry neither key — so a parent skill's loaded instructions or a typed `/name` can still reach them — but none is ever picked by the model reading a bare description, so all thirteen are `name-only` in `skillOverrides`.
