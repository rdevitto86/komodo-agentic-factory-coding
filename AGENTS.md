...
|---|---|---|---|
| Command — produces an artifact | `write-<noun>` | User invokes it to create/refresh a deliverable | `write-repo`, `write-backlog`, `write-readme`, `write-runbook`, `write-changelog`, `write-commit-message` |
| Command — scores, finds, or files | `audit-<noun>` | User invokes it, or the loop invokes it mid-band, to score a diff or walk a diff/repo for defects — findings are filed to `BACKLOG.md` as stories unless `--report` is passed | `audit-backlog`, `audit-bugs`, `audit-change-risk`, `audit-changelog`, `audit-code-quality`, `audit-performance`, `audit-readiness`, `audit-readme`, `audit-sdd`, `audit-prd`, `audit-security`, `audit-simplify`, `audit-testing` |
...
| Autoloaded knowledge — domain facts | `standards-<noun>` | Loaded via `paths`/description, or by name from a skill that needs it; states what is true about a language, tool, process, or external artifact | `standards-go`, `standards-security`, `standards-sdlc`, `standards-worklog` |
...
**Every `standards-<language>`/`standards-<framework>` skill follows the same section order**, so a language skill's shape never has to be re-derived from scratch: Comment discipline → Toolchain → Conventions → domain-specific sections → Testing → Quick-reference fields → `Repo layout — <token>` → `Seed backlog — <token>` → Reference material. A process/rule skill (`standards-cicd`, `standards-sdlc`, `standards-security`, `standards-database`, `standards-worklog`, `standards-specs`) is exempt — this governs only the skills a `.go`/`.py`/`.vue`/etc glob loads. Start a new one from `templates/skills/standards.md.tmpl`; `scripts/validate.sh`'s section-order check enforces it on the skills that already exist.
