...
Typed-only workflow skills: `/generate-backlog` · `/audit-readiness` · `/audit-change-risk` · `/audit-code-quality` · `/audit-changelog` · `/audit-sdd` · `/audit-testing` · `/audit-readme`. These stay `disable-model-invocation: true` — a human decision to normalize a backlog or run an audit should start from the user, not the model's own judgement.

`audit-performance`, `audit-backlog`, `generate-commit-message`, `audit-bugs`, `audit-security`, `audit-simplify`, `generate-repo`, `generate-readme`, and the four mid-loop phases (`workflow-decompose`, `workflow-implement`, `workflow-consolidate`, `workflow-complete`) all carry neither key: `/audit-code-quality` invokes the first internally as part of its conventions pass, `workflow-loop`'s P1 invokes the second before the decompose fork, `/workflow-complete` invokes the third for its message block, `ways/sdlc.md` invokes the next three from P2.3/P2.4, and `workflow-loop`'s P0, `/workflow-consolidate`'s README-refresh step, and `/generate-repo`'s Scaffold/Refresh path all invoke `generate-readme` — a `disable-model-invocation: true` skill cannot be reached by another skill's instructions, only by the user typing its name, and several of these run inside forked, unattended phases so none of them can carry that key. **`disable-model-invocation` is the only thing that controls cross-skill reachability — it says nothing about listing cost.** None of these thirteen is ever chosen by the model reading its description out of a bare, undirected request: each is either named explicitly by an already-loaded parent skill's instructions or typed directly, so all thirteen collapse to `name-only` in `skillOverrides`. `workflow-loop` is the sole exception, kept at full description because it must fire from plain language alone with nothing already loaded to name it. The trade is the same one `generate-repo` makes below: each pays full description cost so it stays callable by name.

**Skill naming follows the same six buckets, front-loaded so related skills tab-complete and sort together:**

| Bucket | Shape | Reason | Examples |
|---|---|---|---|
| Command — produces an artifact | `generate-<noun>` | User invokes it to create/refresh a deliverable | `generate-repo`, `generate-backlog`, `generate-readme`, `generate-sdd`, `generate-adr`, `generate-runbook`, `generate-changelog`, `generate-commit-message` |
| Command — scores, finds, or files | `audit-<noun>` | User invokes it, or the loop invokes it mid-band, to score a diff or walk a diff/repo for defects — findings are filed to `BACKLOG.md` as stories unless `--report` is passed | `audit-backlog`, `audit-bugs`, `audit-change-risk`, `audit-changelog`, `audit-code-quality`, `audit-performance`, `audit-readiness`, `audit-readme`, `audit-sdd`, `audit-security`, `audit-simplify`, `audit-testing` |
...
**`generate-sdd` owns the one frozen spec that lives in the repo; `generate-backlog` and `generate-changelog` own the format of the two mutable records — `standards-worklog` is only the read/write directive shared across both, never their shape.** Splitting the spec from the records means a phase that only records work never pays for the spec template — `/workflow-consolidate` dropped from 3,095 tokens to 1,580. Splitting the two records' formats the same way means a phase touching only one of them never pays for the other's. **The SDD is frozen at approval**, which is why slice status never writes back into SDD §10. **A PRD is a separate, optional artifact — a Google Doc in Drive, read via `standards-prd`, never a repo file and never authored here** — see PRDs and Google Drive below.

**`generate-adr` owns the decision records the SDD points at; `generate-runbook` owns the operational procedures it points at.** Split out for the same reason: editing `sdd.md` should never pay for the ADR or runbook template, and touching a file under `docs/adrs/` or `docs/runbooks/` should never pay for the SDD section skeleton.

**PRDs and Google Drive.** A PRD is optional per code repo, lives as a Google Doc in Drive, and is **read-only from here** — authoring happens with stakeholders outside this toolkit, so no skill creates, edits, or resolves a `NEEDS DECISION` in one. `standards-prd` owns the read contract: the section map, the fetch rule, and the requirement-ID scheme. `standards-docs`' frozen-spec/satellite-record family covers only `docs/sdd.md`, `docs/adrs/`, and `docs/runbooks/` — the PRD is neither colocated nor path-triggered the way that set is.

**There is no fallback tier from the SDD to the PRD.** "Fetch the PRD when local context is thin" is not implementable — the agent judging *thin* is the one that would otherwise invent, so the trigger fires never or always. Each question has exactly one owner instead: the SDD answers *how it works*, the PRD answers *scope, priority, and what counts as success*, and when the PRD is the owner but none exists, the answer comes from the user. **Three fetch points, all in the main session** — `workflow-loop` P0 (§4/§7/§10), `generate-backlog` (§7 priorities, §10 phases), `audit-sdd` (§7 IDs). Every other phase needs none, because `generate-sdd`'s cross-reference contract already puts every requirement ID into SDD §1/§11 — **the IDs are local; only their priority is not.**

**No fork ever reaches Drive.** `planner` and `implementer` declare no MCP tools, so a forked phase cannot fetch even if it wanted to — whatever it needs arrives in `$ARGUMENTS`. Nothing fetches at session start either, the same rule that keeps `context_injector.py` off the bridge. And a fetched PRD is never cached into a repo file: that would be a static reference, which No static references above forbids outright.

**A skill with a procedure half gets a sibling file.** `generate-sdd/authoring.md`, `standards-sdlc/reference.md`, `standards-go/reference.md`, `standards-security/review.md` — the rule stays in `SKILL.md`, the how-to loads only when someone is doing that job.

**Every `standards-<language>`/`standards-<framework>` skill follows the same section order**, so a language skill's shape never has to be re-derived from scratch: Comment discipline → Toolchain → Conventions → domain-specific sections → Testing → Quick-reference fields → `Repo layout — <token>` → `Seed backlog — <token>` → Reference material. A process/rule skill (`standards-cicd`, `standards-sdlc`, `standards-security`, `standards-database`, `standards-worklog`, `standards-docs`) is exempt — this governs only the skills a `.go`/`.py`/`.vue`/etc glob loads. Start a new one from `templates/skills/standards.md.tmpl`; `scripts/validate.sh`'s section-order check enforces it on the skills that already exist.
...
- **Typed-only workflow skills cost nothing** — `disable-model-invocation: true` keeps `/generate-backlog`, `/audit-readiness`, `/audit-change-risk`, `/audit-code-quality`, `/audit-changelog`, `/audit-sdd`, `/audit-testing`, and `/audit-readme` out of the listing entirely.
- **Everything reached only by an explicit name is `name-only`, full stop.** The four mid-loop phases (`/workflow-decompose`, `/workflow-implement`, `/workflow-consolidate`, `/workflow-complete`), `/generate-repo`, `/generate-readme` (also `paths`-triggered on `README.md`, so it still expands in full the instant the model is about to touch that file), and the six audit/commit command skills (`/audit-performance`, `/audit-backlog`, `/generate-commit-message`, `/audit-bugs`, `/audit-security`, `/audit-simplify`) carry neither key — so a parent skill's loaded instructions or a typed `/name` can still reach them — but none is ever picked by the model reading a bare description, so all twelve are `name-only` in `skillOverrides`.
