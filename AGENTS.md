...
|---|---|---|---|
| Command — produces an artifact | `write-<noun>` | User invokes it to create/refresh a deliverable | `write-repo`, `write-readme`, `write-changelog` |
| Command — scores, finds, or files | `audit-<noun>` | User invokes it, or the loop invokes it mid-band, to score a diff or walk a diff/repo for defects — findings are filed to `BACKLOG.md` as stories unless `--report` is passed | `audit-bugs`, `audit-change-risk`, `audit-changelog`, `audit-code-quality`, `audit-performance`, `audit-readiness`, `audit-readme`, `audit-security`, `audit-simplify`, `audit-testing` |
| Command — authors and audits one local doc type | bare `<doc-type>` | One file (or one-file-per-entry) type with both a fixed shape to author and a structural/drift check against it — merging the two into one skill (`<mode>` argument) beats a permanent `write-<x>`/`audit-<x>` pair when the shape and the check are this tightly coupled; the type usually sits under `docs/`, but `backlog` applies the same reasoning to `BACKLOG.md`, which sits at repo root | `sdd`, `prd`, `adr`, `runbook`, `backlog` |
...
- **Typed-only workflow skills cost nothing** — `disable-model-invocation: true` keeps `/audit-readiness`, `/audit-change-risk`, `/audit-code-quality`, `/audit-changelog`, `/audit-testing`, and `/audit-readme` out of the listing entirely.
