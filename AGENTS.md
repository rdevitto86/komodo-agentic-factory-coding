...
Typed-only workflow skills: `/generate-backlog` · `/audit-readiness` · `/generate-readme` · `/audit-change-risk` · `/audit-code-quality`. These stay `disable-model-invocation: true` — a human decision to normalize a backlog, run an audit, or refresh a README should start from the user, not the model's own judgement.

`audit-performance`, `audit-backlog`, `generate-commit-message`, `audit-bugs`, `audit-security`, and `audit-simplify` are the command skills that carry neither key: `/audit-code-quality` invokes the first internally as part of its conventions pass, `workflow-loop`'s P1 invokes the second before the decompose fork, `/workflow-complete` invokes the third for its message block, and `ways/sdlc.md` invokes the last three from P2.3/P2.4 — a `disable-model-invocation: true` skill cannot be reached by another skill's instructions, only by the user typing its name. The trade is the same one `generate-repo` makes below: each pays full description cost so it stays callable by name.
...
- **Typed-only workflow skills cost nothing** — `disable-model-invocation: true` keeps `/generate-backlog`, `/audit-readiness`, `/generate-readme`, `/audit-change-risk`, and `/audit-code-quality` out of the listing. The mid-loop phases (`/workflow-decompose`, `/workflow-implement`, `/workflow-consolidate`, `/workflow-complete`, `/generate-repo`) carry neither key and do pay full description cost — the orchestrator has to reach them by name via the Skill tool. `/audit-performance`, `/audit-backlog`, `/generate-commit-message`, `/audit-bugs`, `/audit-security`, and `/audit-simplify` pay the same cost for the same reason: `/audit-code-quality`, `workflow-loop`'s P1, `/workflow-complete`, and `ways/sdlc.md` reach them by name.
