...
- **Typed-only workflow skills cost nothing** — `disable-model-invocation: true` keeps `/assess-readiness`, `/assess-change-risk`, `/assess-code-quality`, and `/assess-testing` out of the listing entirely. `readme-audit` pays the normal full-description cost instead, on purpose — see the Typed-only line above.
