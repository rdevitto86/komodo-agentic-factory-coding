...
description: C# standards — idioms, typing, error handling, async, testing. Load before reading or writing any .cs or .csproj file.
...
Zero comments, zero XML doc comments. Exception messages lead with a verb phrase and never name the method.

## Comment discipline

`rules-commenting` carries the shared template contract. This language's exempt machine directives, verified against the guard's own list: `#pragma warning disable/restore <rule>` (matches the `pragma` prefix). Anything else — including a `///` XML doc comment on a public member — prompts for approval; the guard scans a triple-slash doc comment the same way it scans any other comment.

## Toolchain

- **Version floor is whatever the project file's `<TargetFramework>` (or `<LangVersion>`) declares.** Read it; never assume a release.
- **`Makefile`'s (or `Taskfile`'s) `verify` target is the merge gate** — `dotnet format --verify-no-changes`, a configured analyzer run (Roslyn analyzers driven by `.editorconfig`, or a named package such as a StyleCop analyzer), `dotnet build`, `dotnet test`, in that order. `context_injector.py` reads this target directly; a repo without it has no gate.
- **Formatting and linting** — `dotnet format` on commit, the project's configured analyzer set as the gate. These are the tools the pre-commit hook runs; `standards-cicd` defines when.
- **Vulnerability scanning** — `dotnet list package --vulnerable` against the resolved lockfile is the gate; triage by whether the vulnerable path is actually reachable, not by CVE score alone. `standards-cicd` defines the gate; the `standards-security` skill states the bar.
- **Nuget packages** via the project's `<PackageReference>` entries at a pinned version — never a local `<ProjectReference>` standing in for a published package outside a solution's own projects.
...
- **Tier definitions, merge/release gates, and coverage floors are owned by `standards-sdlc`** — this section covers C# mechanics only.
...
|---|---|
| Language + framework floor | `.csproj` `<TargetFramework>` |
| Assembly / package name | `.csproj` `<AssemblyName>` |
| Entrypoint | `Program.cs` |
| Run + test | `Makefile` / `dotnet` commands |
...
This skill carries no `Repo layout — <token>` section. **Create is unsupported for C#** in `write-repo` — no repo type token maps here yet. Use `write-repo`'s Scaffold path (doc-pair-only) instead, or add a `Repo layout` section here once a repo type is confirmed.
