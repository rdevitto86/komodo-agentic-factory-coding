...
Model-agnostic quality pass — Read/Grep/Glob/Bash only, no host-specific tooling. Findings only, never fixes — same contract as `audit-bugs`/`audit-security`, just a different lens.
...
4. **Skip correctness and security** — that is `audit-bugs`/`audit-security`'s job, not this one's.
...
Each row becomes one `BACKLOG.md` story unless `--report` is in `$ARGUMENTS`: `- [Sev] <what> · S → \`/audit-simplify <file>\` reports it clear`. Append under the current target state (the first `##` heading) and the domain matching the file's area, or `Cross-Cutting` if none fits — full story-line rules live in `generate-backlog`. `--report` prints the table only; nothing is written.
