...
**Refresh diffs the repo's actual layout and conventions against the language skill's `Repo layout — <token>` tree, and its actual practice against `standards-sdlc` (coverage floors, test tiers), `standards-cicd` (pipeline stages), and `standards-security-api`/`standards-ui-security` wherever the repo visibly departs from one of them.** Only what differs becomes a Deviations line, each tagged with the skill it departs from. Matching it produces zero lines, not a line saying so.
