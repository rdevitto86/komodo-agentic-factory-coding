...
**UI/WCAG rules live in `standards-uiux`**, which already auto-loads on `.tsx`/`.jsx` via its own `paths:`. This skill covers React mechanics only — Tailwind usage, component-rule generalities, and the WCAG bar are `standards-uiux`'s.
