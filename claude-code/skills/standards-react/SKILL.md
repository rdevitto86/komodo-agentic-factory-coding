...
This skill carries no `Repo layout — <token>` section. **Create is unsupported for React** in `git-repo-init` — no repo type token maps here. Use `git-repo-init`'s Scaffold path (doc-pair-only) instead, or add a `Repo layout` section here first.
