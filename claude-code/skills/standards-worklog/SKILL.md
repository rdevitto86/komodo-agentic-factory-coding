...
`BACKLOG.md` is what is still open. `CHANGELOG.md` is what shipped. Both live at the repo root, both are written during a build, and both are mutable — unlike the frozen `docs/prd.md` and `docs/sdd.md`.
...
- **Nothing writes back into the frozen specs.** A `BACKLOG.md` story or `CHANGELOG.md` entry may cite a PRD requirement ID for traceability; `docs/prd.md` and `docs/sdd.md` are otherwise untouched by a build.
