...
- **Internal dependency graph** — the SDK ships it: `dotnet sln list` enumerates the solution's projects and `dotnet list <project> reference` prints that project's `<ProjectReference>` edges. Below project level the SDK ships nothing that lists namespace- or type-level edges. The risk-assessment skill states when to run it and what its output does to the tier.
