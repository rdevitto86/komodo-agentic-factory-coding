---
name: generate-sdd
description: Fixed section template for the frozen SDD — docs/sdd.md, the source of truth for a code repo. Architecture, data model, security, testing, observability, recovery. Load before creating or editing it.
paths: "**/docs/sdd.md, **/sdd.md"
---

# The SDD

`docs/sdd.md` answers *how does it actually work* — full technical detail, but every term is either replaced with plain language or defined in §0 before its first heavy use; never assume the reader is an engineer. Target ~2,800–3,800 words. `standards-docs` states the frozen/entry-point/load-before-touching behavior shared with its satellite records (`docs/adrs/`, `docs/runbooks/`); this file is the SDD's shape.

**The SDD is the source of truth for a code repo.** A PRD is optional and, when it exists, lives in Google Drive (`standards-prd`) — never a local file. The SDD stands on its own: it is never blocked on a PRD existing, and it cites the PRD's requirement IDs only when one has been provided as context.
...
**Companion:** a PRD in Google Drive, when one exists — business scope and requirement IDs live there, not duplicated here. Omit this line entirely when no PRD backs this repo.
...
## 1. Architecture
<System diagram: `![System context](diagrams/system-context.png)`. Component responsibility table. When a PRD exists, cite the requirement ID a component exists to satisfy, e.g. "Control Plane stack (CP1, CP2) — lock table + queue.">
...
## 7. Observability
<Table: signal (logs/metrics/traces/alerts) → what's captured → destination → who's paged. Unset alerting is **NEEDS DECISION**.>

## 8. Operational Requirements
<Performance, scaling, availability targets. Unset targets are **NEEDS DECISION**.>
...
## 12. Risks
<Technical risk table — implementation/operational risk, distinct from the PRD's business-risk table in its §9, when a PRD exists.>

## 13. Open Items
<Known gaps, stated as gaps — never presented as resolved when they aren't.>

## 14. References
<Links: README.md, API contract, diagrams/, and the PRD's Drive link when one exists. Architecture Decision Records: link every file present in `docs/adrs/`, newest first — `generate-adr` owns the template. Runbooks: link every file present in `docs/runbooks/` — `generate-runbook` owns the template. Either directory missing means none exist yet — omit the line, don't create an empty one.>
```

**Requirement IDs:** when a PRD exists, every ID minted in its §7 should appear at least once in §1 or §11 here — the join key back to *why*, and the reason downstream skills can cite an ID without re-fetching the PRD. IDs are minted only in the Drive doc, never here; `standards-prd` owns reading them and `authoring.md` carries the check. With no PRD, the SDD mints and cites no requirement IDs at all.
