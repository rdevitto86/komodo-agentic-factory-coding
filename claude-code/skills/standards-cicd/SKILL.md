...
- **A finding blocks the merge without needing triage.** Load the `standards-security-api` skill (or `standards-ui-security` for a rendered-surface finding) only when deciding whether a finding is genuinely exploitable or an exception is justified — never to run the gate.
