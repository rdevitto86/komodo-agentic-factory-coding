...
- **An exception's shape is owned by `standards-security`** — a committed record with an owner, an expiry, and a compensating control. An expired exception fails the gate again; a permanent suppression is not an exception.
- **A finding blocks the merge without needing triage.** Load the `standards-security` skill only when deciding whether a finding is genuinely exploitable or an exception is justified — never to run the gate.
