---
name: c
description: C standards — toolchain, memory and resource discipline, error handling, testing. Load before reading or writing any .c or .h file. Hosted/application C only — embedded and firmware constraints belong to `cpp`.
...
**Hosted, application-level C.** Firmware, ISR discipline, RTOS, and bare-metal constraints are `cpp`'s domain — load that instead for embedded work; this skill assumes a normal OS process.

## Comment discipline

`comment-rules` carries the shared template contract. This language's exempt machine directives, verified against the guard's own list: `// clang-format off`, `// clang-format on`, `// NOLINT`, `// NOLINTNEXTLINE` (matches the `nolint` prefix). Anything else — including a header-comment block above a function — prompts for approval.
...
- **Coverage floors and merge gates are owned by the `sdlc` skill**; this section covers C mechanics only.
