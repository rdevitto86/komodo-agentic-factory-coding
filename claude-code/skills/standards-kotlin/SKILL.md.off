...
description: Kotlin conventions — null safety, sealed hierarchies, coroutines and Flow, Gradle Kotlin DSL, Android security baseline. Load before writing or reviewing any .kt, .kts, or Gradle Kotlin DSL file.
...
- **The repo's declared test framework is the only one used** — read the test source set rather than mixing frameworks in one suite. Tier definitions and coverage floors are owned by `standards-sdlc`; this section covers mechanics only.
