...
- **High** — requires a valid account but crosses a trust boundary: IDOR, tenant leakage, privilege escalation, missing authz at the service layer.
- **Medium** — needs a non-trivial precondition, a chained bug, or a user interaction: CSRF on a non-critical action, weak crypto with no direct path to plaintext.
...
**Every injection is the same defect: data reaching an interpreter as code.** Identify the interpreter, then confirm the data never reaches it as syntax. (Output-side injection — XSS — is `standards-ui-security`'s review, not this one.)

| Interpreter | The only accepted fix |
|---|---|
| SQL / NoSQL | Parameter binding |
| Shell | Argument array, no shell |
| LDAP / XPath / template | Context-aware escaping or a parameterised API |
...
**Run the scan, do not read the manifest.** The command lives in the language skill (`standards-go`, `standards-typescript`, `standards-python`); this file states what the result must satisfy.
...
For the browser-facing response header set (HSTS, `nosniff`, CSP, `frame-ancestors`), see `standards-ui-security/review.md`.
