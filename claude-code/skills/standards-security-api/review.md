...
**Every injection is the same defect: data reaching an interpreter as code.** Identify the interpreter, then confirm the data never reaches it as syntax. (Output-side injection — XSS — is `standards-security-ui`'s review, not this one.)
...
For the browser-facing response header set (HSTS, `nosniff`, CSP, `frame-ancestors`), see `standards-security-ui/review.md`.
