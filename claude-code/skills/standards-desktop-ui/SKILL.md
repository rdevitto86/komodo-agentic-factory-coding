...
| Concern | Control |
|---|---|
| Keyboard | Full keyboard reach, platform shortcut map, no reserved-key reassignment |
| Menus | Platform menu order, shortcuts displayed, disabled not hidden |
| Windows | Per-window persisted geometry, on-screen restore, multi-DPI, shared document state |
| Density | Pointer-scale layout, OS density respected, hover affordances not sole discovery |
| Context menu | Right-click everywhere contextual, mirrors menu actions |
| Isolation | Context isolation on, host integration off, narrow preload bridge, renderer CSP |
| Protocol handlers | Untrusted arguments, authorised destinations, no shell/path interpolation |
| IPC | Validated and authorised on the privileged side, no generic passthrough, loopback listeners authenticated |
| Updates | Signed and verified before apply, no downgrade, signed and notarised artifact |
| Drag-drop | Content-sniffed type, no navigation on drop, traversal rejected |
