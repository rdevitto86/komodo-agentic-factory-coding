...
Full TS/JS testing standard — the approved stack, file placement, suite structure, runner configuration, and cost control. Go rules live in the `standards-go` skill; Python rules in the `standards-python` skill.

**This file owns mechanics only.** Tier definitions, merge and release gates, and coverage floors are owned by the `standards-sdlc` skill. Which tier runs at which pipeline stage, and on what infrastructure, is owned by the `standards-cicd` skill. Repo shape and service anatomy come from the framework skill in use (`standards-svelte`, `standards-vue`) or the repo's own `AGENTS.md`. Nothing here restates any of them.
...
| Contract | The consumer-driven tool `standards-sdlc` names | Pact ecosystem; the JS binding is the TS half of the same file-based exchange |
| Integration | Ephemeral instances, per `standards-cicd` | Created and destroyed by the run; never a shared long-lived environment |
...
**Suites, helpers, and descriptions** follow the `standards-sdlc` skill. The TS mechanics:
...
The `standards-sdlc` skill owns the policy; these are the Vitest and Playwright mechanics.
