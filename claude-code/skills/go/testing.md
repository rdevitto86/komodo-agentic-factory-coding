...
- **One suite per module under test**, table-driven with `t.Run` subtests. Scenario titles, one level of subtests, never a subtest inside a subtest — see the `sdlc` skill.
...
- Helpers and setup funcs sit at the **bottom of the file, unexported**, under the one permitted banner. Placement across files follows the usage table in the `sdlc` skill.
...
**A one-or-two-line description above a `func Test…` is allowed and optional.** Use it for a corner case or an invariant the name cannot carry, never to paraphrase the identifier. See the `sdlc` skill.
...
Policy is the `sdlc` skill's; these are the Go mechanics for it.
