...
description: Audit CHANGELOG.md against git history for missing/backfillable entries, format drift, and version-bump/tag misalignment, filed as BACKLOG.md stories. Pass --report to skip the write.
argument-hint: [scope] [--report]
---

# Changelog audit

Findings only, never edits `CHANGELOG.md` — `changelog-write` owns the format and is the only skill that writes it. This locates backfill candidates, format inconsistencies, and version/tag misalignment, and hands the fix to the user or a follow-up `/changelog-write` pass, same division of labor `backlog-audit` keeps with `backlog-modify`'s planning/normalize modes.

Every check below tests against `changelog-write`'s own rules (grouping, versioning, tag sync), not rules restated here.

Scoping: **$ARGUMENTS**, minus any `--report` token (default: the whole file)
