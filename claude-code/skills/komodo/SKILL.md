---
name: komodo
description: Drive the Komodo harness from a session: run a task group, check status, plan tasks, lint the backlog, act on a PR. Use when asked to build, ship, or run work end to end.
argument-hint: [run <group> | status | tasks lint | pr respond]
---
...
- Report the outcome with the mandatory three-bucket summary from AGENTS.md.
