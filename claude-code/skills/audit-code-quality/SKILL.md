...
**The catch-all, not a fixer.** `/audit-simplify` finds reuse/simplification/efficiency issues; this scores what's left if nobody gets to them. `/audit-change-risk` is the other single-dimension score the user invokes on their own; this one folds `/audit-performance` in as a standing input instead of leaving performance to be invoked separately.

## Process

1. Read the diff.
2. Invoke `/audit-performance --report` and note its tier — `--report` because this skill's own `Findings → backlog` step already covers the diff; a bare call would file the same diff twice.
...
| Med-High | Drift spanning multiple files in the same package — naming, SDK reuse, or a pattern `/audit-performance` flagged |
...
Score the **highest tier reached by either the convention check or `/audit-performance`** — one Critical-tier file outweighs nine Low-tier ones.
...
**Next:** run /audit-simplify, or address what /audit-performance named — <one-line reason>
```
...
At Med-High or above, file one `BACKLOG.md` story unless `--report` is in `$ARGUMENTS`: `- [Sev] Convention debt: <TIER> — <driving factor> · S → \`/audit-code-quality\` scores Med or below`. Sev maps Med-High→`[M]`, High→`[H]`, Critical→`[C]`. Below Med-High, nothing is filed — there's no action to track. Append under the current target state's `Cross-Cutting` domain — full story-line rules live in `generate-backlog`. `--report` prints the score only; nothing is written.
