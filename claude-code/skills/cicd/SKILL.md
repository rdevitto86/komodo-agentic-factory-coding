...
**`sdlc` owns the target rules** — `-mock` default, `-live` opt-in, PROD never a local target, e2e's local-vs-STG authority, and the local run being the same code as the STG run. Read them there; they are not restated here.
...
- **An exception's shape is owned by `security`** — a committed record with an owner, an expiry, and a compensating control. An expired exception fails the gate again; a permanent suppression is not an exception.
...
|---|---|
| **smoke** | Triggers the hard/soft dependency-backout decision (tier rule: `sdlc`), and starts the inactive→active flip |
