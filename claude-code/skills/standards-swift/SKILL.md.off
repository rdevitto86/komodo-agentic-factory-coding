...
description: Swift conventions — ARC and ownership, optionals, structured concurrency, API design guidelines, SPM. Load before writing or reviewing any .swift file or Package.swift manifest.
...
- **Internal dependency graph** — SwiftPM ships it: `swift package describe` lists every target and the targets it depends on, `swift package show-dependencies` the package-level tree. Below target level the toolchain ships nothing. `assess-change-risk` states when to run it and what its output does to the tier.
...
- **The repo's declared test framework is the only one used** — read the manifest's test target rather than mixing frameworks in one suite. Tier definitions and coverage floors are owned by `standards-sdlc`; this section covers mechanics only.
