...
`standards-comments` owns the rules and loads on the same files this skill does — read it there, not here. In short: every function gets one line saying what it does, public or private; everything else gets nothing unless the code cannot say it; `comments.py apply` is the only write path. A Javadoc block above a method already satisfies the rule. **Deleting a comment you did not add is not licence taken by refactoring** — restore it, or say in `## Notes` why it went. **Applies to every comment syntax**, not just `//` — block comments (`/* */`) and Javadoc are scanned the same way.
...
- **Internal dependency graph** — `jdeps` ships with the JDK: run it over the compiled output for package edges, `-verbose:class` for class granularity, `-filter:none` to keep same-package edges. It reads classes, not sources, so build first. Module-to-module edges come from the build itself (`mvn dependency:tree`, `./gradlew :<module>:dependencies`). `assess-change-risk` states when to run it and what its output does to the tier.
...
This skill carries no `Repo layout — <token>` section. **Create is unsupported for Java** in `git-repo-init` — no repo type token maps here yet. Use `git-repo-init`'s Scaffold path (doc-pair-only) instead, or add a `Repo layout` section here once a repo type is confirmed.
