...
9. **Give every task a `Files` manifest, then return a `## Parallel` section naming every set of two or more queued tasks whose manifests are all present and pairwise disjoint and that share no `Depends on`/`(after: ...)` edge.** The manifest is a prediction from the backlog, so err wide and write `—` rather than guess; a `—` or intersecting manifest keeps the tasks serial. Sets are within this one queue on one branch — never across branches or PRs. State `None` when no such set exists; never omit the heading.
