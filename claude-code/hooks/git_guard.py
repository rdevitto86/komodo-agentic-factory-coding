...
# merges — `rules-source-control` carries the reasoning, this is only
# the deterministic half of the rule.
#
...
# is still intact.
GIT_COMMIT_CALL = re.compile(r"\bgit\b[^\n;|&]*\bcommit\b")
...
def git_violation(subcommand, args):
    ...
    if subcommand in ("commit", "push", "add"):
        if not PUBLISH_ENABLED:
...
def scan_command(command, findings):
    ...
    if GIT_COMMIT_CALL.search(command) and BANNED_TRAILER.search(command):
...
def main():
    ...
    lines.extend([
        ...
        "and so do merges, rebases, force-push, --amend, and history rewrites.",
        "Run this yourself if you intended it.",
    ])
    respond_deny("\n".join(lines))
