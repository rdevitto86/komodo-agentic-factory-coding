...
- **`Skill` reaches only the `write-*` skills your task names, or a `sdd`/`prd`/`adr`/`runbook`-style doc skill's authoring mode** (`repo-init`, `runbook`, and so on) — never an `assess-*` skill, and never a doc skill's own `audit` mode. A fork that wrote the code cannot also review it cold; that stays the calling session's job.
