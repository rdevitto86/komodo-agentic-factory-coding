...
- **Read-only git** — `log`, `diff`, `show`, `status`, `blame`, `ls-files`. Never commit, stage, branch, or push. `git_guard.py` enforces this by identity — any other git subcommand from this agent is denied.
...
**Comments, last.** Once every `Done when` command passes, run `python3 ~/.claude/hooks/comments.py check --json`. **Every function you wrote gets a comment saying what it does** — public and private, in any language — unless it is in a test file, has a one-statement body, has no body, or already carries a docstring or JSDoc block. `check` names each one as `FUNC_UNDOCUMENTED`. **Everywhere else the default is zero proposals**, and only `write-comments/SKILL.md`'s bar gets you one. Never cite a version, an SDK release, a `PRD`/`SDD`, a ticket, or this conversation — that is `EXTERNAL_REF` and `apply` refuses it. No restatements, no hypotheticals, no call-site reasoning, no point-in-time context; two lines above a function, one above everything else. Read `~/.claude/skills/standards-comments/SKILL.md` before drafting — the bar, the bans, and the worked examples are there, not here. Pipe whatever you draft through `python3 ~/.claude/hooks/comments.py apply` — never write a comment with `Edit`/`Write` directly.

**Re-run `check` until it exits 0.** A `MISSING` finding on a function you wrote is never skippable — write the comment. The only finding you may leave is an `INVALID` on a line that was already in the file when you started, and you list it in `## Notes` with its rule name.
