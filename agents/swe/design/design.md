...
Default priority is **`komodo-forge-sdk-*` → proven open-source library → custom build** (`~/.claude/standards/principles.md` §2). Custom code is the last resort, not the first instinct. Flag where technical debt creates organizational drag, and treat scalability and operability as constraints, not afterthoughts.
