...
- One reviewer pass per group carrying bug, security, test-gap, simplify, narrative-comment, and undocumented-nonobvious classes with a severity scale. Floor findings get one repair; the rest are filed to the backlog by code.
...
### Changed
- `claude-code/` is a thin adapter installed by copy. `AGENTS.md` is ~40 lines: working, git, comments, and writing for a human. Seven agent profiles carry model, effort, and a short body. Three procedure skills (`komodo`, `backlog`, `review`) plus one thin `standards-*` pointer per rules file, every one `name-only`, keep the always-on listing near 900 tokens against a 1,500 budget.
- `settings.policy.json` replaces the tracked `settings.json`. It carries only `permissions`, `hooks`, and `skillOverrides`; `komodo install` merges those keys into `~/.claude/settings.json` and keeps every personal key. `settings.json` is gitignored.
